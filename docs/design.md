...
A git orchestrator for managing N dotfile repos with one tool: clone, symlink, save (commit + push) across all repos in a single command. Designed for the per-feature-repo pattern.
